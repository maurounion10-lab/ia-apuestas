...
` : ''}
      ...
      <div class="pred-result-stamp ${isHighConfWin ? 'stamp-highconf' : isFinishedWin ? 'stamp-win' : isFinishedLoss ? 'stamp-loss' : 'stamp-pending'}">
        <span class="pred-result-stamp-icon">${isHighConfWin ? `<svg viewBox="0 0 44 44" xmlns="http://www.w3.org/2000/svg" aria-label="Acertado Máxima" width="44" height="44"><defs><radialGradient id="gr_hc" cx="0.35" cy="0.25" r="0.9"><stop offset="0" stop-color="#fff2a8"/><stop offset="0.5" stop-color="#ffcf1f"/><stop offset="1" stop-color="#a56a00"/></radialGradient><linearGradient id="gs_hc" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="rgba(255,255,255,0.6)"/><stop offset="0.6" stop-color="rgba(255,255,255,0)"/></linearGradient></defs><circle cx="22" cy="22" r="20" fill="url(#gr_hc)" stroke="#7a4a00" stroke-width="1.5"/><circle cx="22" cy="22" r="18" fill="url(#gs_hc)"/><path d="M13 22.5 L19.5 29 L31.5 16.5" fill="none" stroke="#4a2500" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round"/></svg>` : isFinishedWin ? `<svg viewBox="0 0 44 44" xmlns="http://www.w3.org/2000/svg" aria-label="Acertado" width="44" height="44"><defs><radialGradient id="gr_win" cx="0.3" cy="0.3" r="0.85"><stop offset="0" stop-color="#4dff8f"/><stop offset="0.55" stop-color="#00d968"/><stop offset="1" stop-color="#008a42"/></radialGradient><linearGradient id="gs_win" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="rgba(255,255,255,0.55)"/><stop offset="0.55" stop-color="rgba(255,255,255,0)"/></linearGradient></defs><circle cx="22" cy="22" r="20" fill="url(#gr_win)" stroke="#004a1e" stroke-width="1.5"/><circle cx="22" cy="22" r="18" fill="url(#gs_win)"/><path d="M13 22.5 L19.5 29 L31.5 16.5" fill="none" stroke="#fff" stroke-width="3.6" stroke-linecap="round" stroke-linejoin="round"/></svg>` : isFinishedLoss ? '❌' : '⏳'}</span>
      ...
      <!-- ── Fila de acciones: Compartir · Análisis · Comentar · Likes ── -->
      <div class="pred-action-row">
        <button class="pred-action-btn btn-share" onclick="sharePick('${p.home}','${p.away}','${p.rec}',${_bvr},'${p.league}')" title="Compartir este pick">
          📤 <span>Compartir</span>
        </button>
        ${p.insight
          ...
          : (() => {
              ...
              return `<a class="pred-action-btn btn-twitter" href="https://twitter.com/intent/tweet?text=${_tweetTxt}" target="_blank" rel="noopener" title="Twitear esta apuesta">
              🗣️ <span>Twitear</span>
            </a>`;
            })()
        }
        <button class="pred-action-btn btn-forum" onclick="openPickForum('${p.home.replace(/'/g,"\\'")}','${p.away.replace(/'/g,"\\'")}',{home:'${p.home.replace(/'/g,"\\'")}',away:'${p.away.replace(/'/g,"\\'")}',league:'${(p.league||'').replace(/'/g,"\\'")}',rec:'${(p.rec||'').replace(/'/g,"\\'")}',bvrText:'${(_bvrText||'').replace(/'/g,"\\'")}'})" title="Ver comentarios">
          💬 <span>Comentar</span>
        </button>
        <button class="pred-action-btn btn-like${_isPickLiked(idx) ? ' liked' : ''}" id="plbtn-${idx}" onclick="togglePickLike(${idx}, ${source.length})" title="Me gusta este pick">
